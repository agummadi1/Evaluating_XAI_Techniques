...
def get_and_preprocess_mems(params):
    PROTECTED_CLASS = params.protected_class
    UNPROTECTED_CLASS = params.unprotected_class
    POSITIVE_OUTCOME = params.positive_outcome
    NEGATIVE_OUTCOME = params.negative_outcome	
    ...
    req_cols = ['x','y','z','label']  # mems - take 3; d7 - top 5 of desc acc

    cols = req_cols
    # Load your dataset
    df = pd.read_csv('/N/u/agummadi/BigRed200/mems_dataset.csv', usecols=req_cols)
    ...
    # df ['Label'] = y 
    # X = X.drop(["Label"], axis=1)
    # print(label.index('failure'))
    # # X[' Flow Duration'] = [label.index('BENIGN') if v > 10000 else label.index('Dos/Ddos')  for v in X[' Flow Duration'].values]
    X['z'] = [0 if v < 0.7 else 1  for v in X['z'].values]
  #  want to pass the wrong explanation
    # < 0.7 is failure
...
def get_and_preprocess_iot7(params):
    PROTECTED_CLASS = params.protected_class
    UNPROTECTED_CLASS = params.unprotected_class
    POSITIVE_OUTCOME = params.positive_outcome
    NEGATIVE_OUTCOME = params.negative_outcome	
    ...
    req_cols = ['HH_L5_magnitude','HH_L1_magnitude','HH_L3_magnitude','HH_L3_mean','HH_L5_weight','label']  # mems - take 3; d7 - top 5 of desc acc

    cols = req_cols
    # Load your dataset
    df = pd.read_csv('/N/u/agummadi/BigRed200/device7_top_20_features.csv', usecols=req_cols)

    X = df.drop(columns=['label'])
    y = df['label']

    X['HH_L5_magnitude'] = [0 if v < 700 else 1  for v in X['HH_L5_magnitude'].values]
